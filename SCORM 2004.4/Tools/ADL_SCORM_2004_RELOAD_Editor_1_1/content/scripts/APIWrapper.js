...
//window.alert("Showing message is necessary here to proceed the process. Should find the reason. We don't need this message at all.");
...
function doGetValue(name)
{
   ...
   else
   {
      ...
      else
      {
         ...
         adl_qd_debug( "LMSGetValue: <b>" + name + "</b> = " + value.toString() );
      }
   }
}
...
function doSetValue(name, value)
{
    adl_qd_debug( "LMSSetValue: <b>" + name + "</b> = " + value );
   var api = getAPIHandle();
   if (api == null)
   {
...
function ErrorHandler()
{
   ...
   if (errCode != _NoError)
   {
      ...
      adl_qd_debug( "<font color=\"red\">" + errDescription + "</font>" );
   }
...
function getAPI()
{
   ...
   {
	   var numOfObj = parseInt( doGetValue("cmi.objectives._count") );
	   var objectiveLocation = -1;

	   for ( var i=0; i < numOfObj; i++ ) 
	   {
           adl_qd_debug( "looking for objectives ... " );
